...
import areaBgPic from "@/views/echarts/map/map-3d/area_bg.jpg"; // require('@/assets/images/area_bg.jpg')
import "echarts-gl";
// https://juejin.cn/post/7311959845539872768
...
function getOption(data) {
  ...
  return {
    ...
    geo3D: {
      ...
      realisticMaterial: {
        detailTexture: areaBgPic, // 材质细节的纹理贴图
        textureTiling: 1 // 纹理贴图在平面上的重复次数
      },
      emphasis: {
        label: {
          show: true,
...
// 从数组中随机抽取length个不重复的数据
function getArrayRandomData(array = [], length = 10) {
  if (length > array.length) {
    console.log("Sample size exceeds array length");
    return [];
  }
  return [...array]
...
// 从对象中随机抽取length个不重复的属性和属性值
function getObjectRandomData(obj = {}, length = 10) {
...
async function getData() {
  try {
    ...
    const barData = Object.entries(getObjectRandomData(geoCoordMap)).map(([key, value]) => {
      return {
        name: key,
        value: value.concat(Math.floor(Math.random() * 1000))
      };
    });
    const scatterData = Object.entries(getObjectRandomData(geoCoordMap)).map(([key, value]) => {
      return {
        name: key,
        value: value.concat(Math.floor(Math.random() * 1000))
      };
    });
    const linesData = Array.from({ length: 10 }, () => Object.values(getObjectRandomData(geoCoordMap, 2)));
    // const linesData = [
