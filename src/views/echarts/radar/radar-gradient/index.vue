...
// 根据雷达图的数据，计算指标的最大值，从而获得渐变方向。注意：只能返回向左、向上、向左上等8个方向
function getLinearGradient(data = [], startAngle = 90, clockwise = false) {
  if (!data || data.length <= 1) return { x: 0, y: 0, x2: 1, y2: 1 };

  // 找到数据最大值对应的索引
  const maxValue = Math.max(...data);
  const maxIndex = data.indexOf(maxValue);

  // 计算最大值在数据中的相对角度 (0-360°)
  const angle = maxIndex * 360 / data.length;

  // 根据位置计算渐变方向。clockwise表示是否是顺时针方向，默认为逆时针
  const getDirectionByPosition = () => {
    // 计算目标角度：根据顺时针/逆时针调整方向
    let targetAngle;
    if (!clockwise) {
      // 逆时针：起始角度 + 位置 * 360度，角度转换为弧度
      targetAngle = (startAngle + angle) * Math.PI / 180;
    } else {
      // 顺时针：起始角度 - 位置 * 360度，角度转换为弧度
      targetAngle = (startAngle - angle) * Math.PI / 180;
    }

    // 计算目标点坐标（在单位圆上）,并保留1位小数
    const targetX = Number.parseFloat(Math.cos(targetAngle).toFixed(1)); // x轴方向向右
    const targetY = Number.parseFloat(-Math.sin(targetAngle).toFixed(1)); // 负号是因为y轴向下

    const x = targetX < 0 ? 1 : 0;
    const y = targetY < 0 ? 1 : 0;
    const x2 = targetX > 0 ? 1 : 0;
    const y2 = targetY > 0 ? 1 : 0;
    return { x, y, x2, y2 };
  };

  return getDirectionByPosition();
}
...
function getOption(data) {
  ...
  const startAngle = 90;
  const direction = getLinearGradient(seriesData?.length && seriesData[1].data.value || [], startAngle);
    ...
    series: [
      {
        ...
        areaStyle: {
          ...
          color: {
            ...
            r: 1.0,
            colorStops: [
              { offset: 0, color: `${chartColor[0]}00` }, // 0% 处的颜色
              {
                offset: 1,
                color: chartColor[0] // 100% 处的颜色
              }
            ],
        ...
        seriesData: [
          {
            name: "测试组1",
            data: {
              name: "区域渐变色", // 图表名称，如果需要显示legend，则需要设置name属性
