...
function getOption(data) {
  const { title = "", subtitle, axisName, seriesData } = data;
  ...
  return {
    ...
    polar: {
      center: ["50%", "50%"], // 极坐标系的中心（圆心）坐标，数组的第一项是横坐标，第二项是纵坐标。支持设置成百分比
      radius: "100%", // 极坐标系的半径。支持number，string(例如，'20%')，Array.<number|string>
    ...
    radiusAxis: {
      ...
      polarIndex: 0, // 该极坐标系的索引，默认使用第一个极坐标系
    ...
    // 极坐标系的角度轴
    angleAxis: {
      show: true, // 是否显示角度轴，默认为true
      type: "value", // 坐标轴类型，默认为'value'，可选为：'value'数值轴，'category'类目轴，'time'时间轴，'log'对数轴
      polarIndex: 0, // 该角度轴所在的极坐标系的索引，默认使用第一个极坐标系
      startAngle: 90, // 起始角度，默认为 90 度，即圆心的正上方。0 度为圆心的正右方
      // endAngle: 90, // 结束角度，默认为 90 度，即圆心的正上方。0 度为圆心的正右方
      clockwise: true, // 刻度增长是否按顺时针方向，默认为 true。设置为 false 时刻度按逆时针方向增长
      boundaryGap: ["20%", "20%"], // 数值轴留白策略，可选为：'true'留白策略，'false'不留白，'百分比'表示留白宽度百分比
      min: 0, // 坐标轴刻度最小值，可选：'dataMin' | 'dataMax' | number | Function
      max: 100, // 坐标轴刻度最大值，可选：'dataMin' | 'dataMax' | number | Function
      startValue: 0, // 坐标轴的起始值。
      splitNumber: 5, // 坐标轴的分割段数，默认值：5。需要注意的是这个分割段数只是个预估值，最后实际显示的段数会在这个基础上根据分割后坐标轴刻度显示的易读程度作调整。
      minInterval: 0, // 自动计算的坐标轴最小间隔大小，例如可以设置成1保证坐标轴分割刻度显示成整数。只在数值轴或时间轴中（type: 'value' 或 'time'）有效。
      maxInterval: 0, // 自动计算的坐标轴最大间隔大小，例如可以设置成30保证坐标轴分割刻度最大为30。只在数值轴或时间轴中（type: 'value' 或 'time'）有效。
    ...
    series: [
      {
        type: "bar",
        coordinateSystem: "polar", // 该系列使用的坐标系，极坐标系
        roundCap: true,
        barWidth: 30,
        silent: true,
        showBackground: true,
        ...
        stack: (seriesData?.length && seriesData[0].name) || "",
        data: (seriesData?.length && seriesData[0].data) || [0]
      },
      // 顶部圆环装饰
      {
        type: "bar",
...
async function getData() {
  try {
    ...
    const res = {
      code: "0",
      data: {
        title: "极坐标系(饼图效果，环形进度图)",
        subtitle: "62.35%",
        axisName: "测试数据",
        seriesData: [
          {
            name: "测试数据1",
            data: [63.2] // [Math.floor(Math.random() * 80)]
          }
        ]
      }
    };
...
<template>
  <div class="chart-container">
    <h2>极坐标系: coordinateSystem:'polar'，用于绘制雷达图、极坐标图等。</h2>
    <h3>
      它由角度轴和径向轴组成，可以通过 polar、angleAxis 和 radiusAxis
      配置项进行设置。
    </h3>
